...
import { NetworkTableColumns } from '@/components/Tables/Networks'
// Pagination
const page = ref(1)
...
// Data
const { data, status } = await useFetch('/api/networks', {
    query: {
...
<template>
    <div>
        <UCard class="w-full" :ui="{
            ...
            <UTable v-model="selected" :columns="NetworkTableColumns" :rows="data.data" :loading="status === 'pending'">
                <template #password-data="{ row }">
                    {{ row.password ? row.password : 'None' }}
                </template>
