...
import { DeviceTableColumns } from './Tables/Devices';
...
const { data, status } = await useFetch('/api/devices', {
    query: {
    },
    default: () => {
        return {
...
<template>
    <div>
        <UCard class="w-full" :ui="{
            ...
            <UTable v-model="selected" :columns="DeviceTableColumns" :rows="data?.data" :loading="status === 'pending'">
                <template #image-data="{ row }">
                      <UAvatar size="2xl" :src="row.image" />
                </template>
            ...
            <template #footer>
                <div class="flex flex-wrap justify-between items-center">
                    ...
                    <UPagination v-model="page" :page-count="pageCount" :total="data.data?.length" :ui="{
